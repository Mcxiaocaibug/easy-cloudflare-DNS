...
<div align="center">
  ...
  <img src="https://img.shields.io/badge/Database-SQLite3%20%2F%20MySQL-orange.svg" alt="Database">
...
| **MySQL** | 5.7+ / 8.0+ | 关系型数据库（Docker） | 企业级，高并发 |
...
使用 Docker 可以快速部署，无需手动配置环境，支持 MySQL 数据库。

#### 方式 1: 使用预构建镜像（最快）

```bash
# 1. 下载配置文件
wget https://raw.githubusercontent.com/976853694/cloudflare-DNS/main/docker-compose.ghcr.yml -O docker-compose.yml
wget https://raw.githubusercontent.com/976853694/cloudflare-DNS/main/env.example -O .env
...
**注意**: 将 `docker-compose.ghcr.yml` 中的 `YOUR_GITHUB_USERNAME/REPO_NAME` 替换为实际的镜像地址。
...
- ✅ **一键部署**: 自动配置所有环境依赖
- ✅ **MySQL 数据库**: 使用 MySQL 替代 SQLite，性能更优
- ✅ **环境变量配置**: 所有配置通过环境变量管理
- ✅ **支持外部数据库**: 可连接已有的 MySQL 数据库
- ✅ **自动初始化**: 首次启动自动创建管理员账户
- ✅ **数据持久化**: 使用 Docker Volume 持久化数据

#### 详细文档

📖 完整的 Docker 部署文档请查看: [README.Docker.md](./README.Docker.md)
